...
namespace STL.PL
{
    public partial class SalesCommissionMaintenance : CommonForm
    {
        ...
        private DataTable commTypes= new DataTable();        // RI
        private DataTable upliftRates = new DataTable();
        private DataTable userRoles = new DataTable();      // #11203
        private DataSet uplift = new DataSet();             // #11203
        private BindingSource boundUpliftRates;             // #11203
       ...
       // public string commItemStr;
        ...
        private bool commRepoItem = false;
        private bool loaded = false;
        private bool upliftError = false;
        private bool upliftUpdated = false;
        private int selectedRole = 0;               // #11203
        private bool urloaded = false;
        

        public SalesCommissionMaintenance()
        {
            InitializeComponent();           
        ...
        public SalesCommissionMaintenance(Form root, Form parent)
        {
            FormRoot = root;
            FormParent = parent;
            ...
            commRepoItem = Convert.ToBoolean(Country[CountryParameterNames.ComRepoItem]);       // RI
            
            DataRow newRow = commTypes.NewRow();
            commTypes.Columns.Add("ComDisplay");
            commTypes.Columns.Add("ComValue");

            if (Convert.ToBoolean(Country[CountryParameterNames.RIDispCatAsDept]))       // RI Dropdown to show Category OR Department
            {
                newRow["ComDisplay"] = "Product Department";
            }
            else
            {
                newRow["ComDisplay"] = "Product Category";
            }
            newRow["ComValue"] = "Product Category";                // Dropdown Value always Category
            commTypes.Rows.Add(newRow);
            DataRow newRow2 = commTypes.NewRow();
            newRow2["ComDisplay"] = "Product Class";
            newRow2["ComValue"] = "Product Class";
            commTypes.Rows.Add(newRow2);
            DataRow newRow3 = commTypes.NewRow();
            newRow3["ComDisplay"] = "Product SubClass";
            newRow3["ComValue"] = "Product SubClass";
            commTypes.Rows.Add(newRow3);
            DataRow newRow4 = commTypes.NewRow();
            newRow4["ComDisplay"] = "Product SKU";
            newRow4["ComValue"] = "Product SKU";
            commTypes.Rows.Add(newRow4);
            DataRow newRow5 = commTypes.NewRow();
            newRow5["ComDisplay"] = "Product";
            newRow5["ComValue"] = "Product";
            commTypes.Rows.Add(newRow5);
            DataRow newRow6 = commTypes.NewRow();
            newRow6["ComDisplay"] = "Terms Type";
            newRow6["ComValue"] = "Terms Type";
            commTypes.Rows.Add(newRow6);
            DataRow newRow7 = commTypes.NewRow();           // #9785
            newRow7["ComDisplay"] = "All Products";
            newRow7["ComValue"] = "All Products";
            commTypes.Rows.Add(newRow7);

            DataView dvcommTypesView = new DataView(commTypes);
            drpCommItem.DataSource = dvcommTypesView;
            drpCommItem.ValueMember = "ComValue";
            drpCommItem.DisplayMember = "ComDisplay";
            ...
            btnSaveUplift.Enabled = false;      // #10951
            // Initial Load
        ...
        private void tcMain_SelectionChanged(object sender, EventArgs e)
        {
            if (upliftError == false)
            {
                ...
                if (tbUpliftRates.Selected)
                {
                    LoadUpliftRates();
                    btnSaveUplift.Visible = true;
                    btnClear.Visible = false;
                }
            ...
            {
                tcMain.SelectedTab = tbUpliftRates;
                ((MainForm)this.FormRoot).statusBar1.Text = "Clear error on this screen before proceeding";
            }
        }
        ...
        {
            commItemStr = Convert.ToString(drpCommItem.SelectedValue);          // #9785
            ClearEntryFields();
            ClearErrors();
            ...
            //commItemStr = Convert.ToString(drpCommItem.SelectedValue);          // RI
            DateTime selectDate = dtpCommDate.Value;
            commItemLabel.Text = Convert.ToString(drpCommItem.Text);            // RI
            ...
            //commRepoItem = false; //testing only
            //commPerAccType = false; //testing only

            //show repo commission text boxes
            repoGroup.Visible = commRepoItem && commItemStr != "Terms Type";
            tbCommPcentCashRepo.Visible = commPerAccType;
           ...
           if (commItemStr != null)
            {
                ...
                dgvCommissions.Columns[CN.ItemText].HeaderText = Convert.ToString(drpCommItem.Text);        // RI
                dgvCommissions.Columns[CN.ItemText].Width = 85;
                ...
                if (commItemStr != "All Products" && commItemStr != "Terms Type")      // #9785   
                {
                    dgvCommissions.Columns["CommissionType"].Width = 45;
                    dgvCommissions.Columns["CommissionType"].Visible = false;
                }
               ...
               // if (commPerAccType == true && commItemStr != "Terms Type")
                if (commItemStr != "Terms Type")
                {
                    ...
                    dgvCommissions.Columns[CN.RepoPercentage].HeaderText = GetResource("T_CreditPctRepo");
                    dgvCommissions.Columns[CN.RepoPercentageCash].HeaderText = GetResource("T_CashPctRepo");
                    dgvCommissions.Columns[CN.RepoPercentage].Width = 65;
                    dgvCommissions.Columns[CN.RepoPercentageCash].Width = 65;
                    dgvCommissions.Columns[CN.RepoPercentage].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dgvCommissions.Columns[CN.RepoPercentageCash].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    // Set visibility of Repo percentage column based on Country parameter
                    dgvCommissions.Columns[CN.RepoPercentage].Visible = commRepoItem;
                    dgvCommissions.Columns[CN.RepoPercentageCash].Visible = commRepoItem && commPerAccType;
                    // set visibility of Cash percentage text box,checkbox & labels
                    tbCommPcentCash.Visible = commPerAccType;
                    cbCopyCredit.Visible = commPerAccType;
                    Cash_Label.Visible = commPerAccType;
                    Credit_Label.Visible = commPerAccType;
                }
                else                    
                {
                    ...
                    dgvCommissions.Columns[CN.RepoPercentageCash].Visible = false;      // RI
                    dgvCommissions.Columns[CN.RepoPercentage].Visible = false;          // RI
                    dgvCommissions.Columns[CN.RepoValue].Visible = false;               // RI
                    if (commItemStr == "Terms Type")
                    {
                ...
                ((MainForm)this.FormRoot).statusBar1.Text = " ";
                if (commItemStr != "All Products")          // #9785
                {
        ...
        private void ClearEntryFields()
        {
            ...
            tbCommPcentRepo.Text = "";
            tbCommPcentCashRepo.Text = "";            
            ...
            tbSpiffPcentRepo.Text = "";     // RI
            tbSpiffValueRepo.Text = "";
        ...
        private void dgvCommissionsRowSelected_click(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                {
                    commItemLabel.Text = Convert.ToString(((DataView)dgvCommissions.DataSource)[index]["CommissionType"]);            // #9785
                    ...
                    tbCommPcentRepo.Text = Convert.ToString(((DataView)dgvCommissions.DataSource)[index][CN.RepoPercentage]);
                    tbCommPcentCashRepo.Text = Convert.ToString(((DataView)dgvCommissions.DataSource)[index][CN.RepoPercentageCash]);
            ...
            try
            {
...

                {
                    ...
                    if (rowViewFound != null)
                    {
                        ...
                        rowViewFound[CN.RepoPercentage] = tbCommPcentRepo.Text;             // RI
                        rowViewFound[CN.RepoPercentageCash] = tbCommPcentCashRepo.Text;     // RI
                        if (commItemStr == "Terms Type")
                    ...
                    else
                    {
                        ...
                        rowView[CN.RepoPercentage] = Convert.ToDouble(tbCommPcentRepo.Text);            // RI
                        rowView[CN.RepoPercentageCash] = Convert.ToDouble(tbCommPcentCashRepo.Text);
                        if (commItemStr == "Terms Type")
                        {
                            rowView[CN.Value] = Convert.ToDouble(tbCommValue.Text);
                            rowView[CN.RepoValue] = Convert.ToDouble(tbCommValue.Text);                 // RI
                        }
                        //else
                        ...
                        if (commItemStr != "Terms Type")                //#17857
                        {
                            rowView["CommissionType"] = commItemLabel.Text;         // #9875
                        }
        ...
        {
            bool valid = true;
            // #9785 save actual commission type if "All Products" selected
            var commItem = commItemStr;
            if (commItemStr == "All Products")
            {
                commItemStr=Convert.ToString(commItemLabel.Text);
            }
            ...
            if (commItemStr == "Terms Type")
            {
                ...
                if (tbCommValue.Text.Trim().Length == 0)
                    tbCommValue.Text = "0";
                if (tbCommPcentCashRepo.Text.Trim().Length == 0)                // RI
                    tbCommPcentCashRepo.Text = "0";
                if (tbCommPcentRepo.Text.Trim().Length == 0)                    // RI 
                    tbCommPcentRepo.Text = "0";
            ...
            //IP - 22/05/08 - UAT(454) v5.1 - Should not be able to enter a 'Product Category' > 3 characters.
            //jec moved to here 11/06/08 UAT
            if (tbCommItem.Text.Length > 3 && commItemStr == "Product Category")
            {
            ...
            // jec 02/07/08 UAT468 - Product category must be numeric
            if (commItemStr == "Product Category" &&            // UAT597   jec 21/05/09
                 (!IsStrictNumeric(tbCommItem.Text) || tbCommItem.Text.Trim().Length == 0))
            {
                errorProvider1.SetError(this.tbCommItem, GetResource("M_NUMERIC"));
                valid = false;
            }
            ...
            // Product Level (Class) must be 2 or 3 characters
            if (tbCommItem.Text.Length != 3 && tbCommItem.Text.Length != 2 && commItemStr == GetResource("T_PRODUCTCLASS"))        // RI
            {
                errorProvider1.SetError(this.tbCommItem, GetResource("M_PRODUCTCLASSLEN"));
                valid = false;
            }
            // Product SubClass) must be 3 to 5 characters
            if ((tbCommItem.Text.Length < 3 || tbCommItem.Text.Length > 5) && commItemStr == GetResource("T_PRODUCTSUBCLASS"))        // RI
            {
                errorProvider1.SetError(this.tbCommItem, GetResource("M_PRODUCTSUBCLASSLEN"));
                valid = false;
            }
            ...
            // Product must not be > 12 characters
            if (tbCommItem.Text.Length > 12 && commItemStr == GetResource("T_PRODUCT"))
            {
                errorProvider1.SetError(this.tbCommItem, GetResource("M_PRODUCTLEN"));
                valid = false;
            }

            // Product SKU must not be > 8 characters
            if (tbCommItem.Text.Length > 8 && commItemStr == GetResource("T_PRODUCTSKU"))
            {
                errorProvider1.SetError(this.tbCommItem, GetResource("M_PRODUCTSKULEN"));
                valid = false;
            }
            ...
            {
                if (!IsStrictNumeric(tbCommPcentRepo.Text) || tbCommPcentRepo.Text.Trim().Length == 0)
                {
                    errorProvider1.SetError(this.tbCommPcentRepo, GetResource("M_NUMERIC"));
                    valid = false;
                }

                if (!IsStrictNumeric(tbCommPcentCashRepo.Text) || tbCommPcentCashRepo.Text.Trim().Length == 0)
                {
                    errorProvider1.SetError(this.tbCommPcentCashRepo, GetResource("M_NUMERIC"));
                    valid = false;
                }
            }
            ...
            if (valid)
            {
                ...
                if (commItemStr != "Terms Type")
                {
                    if (Convert.ToDouble(tbCommPcentRepo.Text) > maxCommRate)
                    {
                        errorProvider1.SetError(this.tbCommPcentRepo, GetResource("M_COMMISSIONRATEEXCEEDED", new object[] { maxCommRate }));
                        valid = false;
                    }
                    if (Convert.ToDouble(tbCommPcentCashRepo.Text) > maxCommRate)
                    {
                        errorProvider1.SetError(this.tbCommPcentCashRepo, GetResource("M_COMMISSIONRATEEXCEEDED", new object[] { maxCommRate }));
                        valid = false;
                    }
                }
            ...
            if (valid)
            {
                ...
                {
                    if (DialogResult.OK == ShowInfo("M_PRODUCTNOTEXIST", new object[] { commItemLabel.Text, this.tbCommItem.Text }, MessageBoxButtons.OKCancel))
                        valid = true;
                    else
                        valid = false;
                }
            ...
            // #9875 - restore "All Products" 
            if (commItem == "All Products")
            {
                commItemStr = commItem;
            }
        ...
        private void SetScreen()
        {
            ...
            //if (commPerAccType == false)
            //{
            //    if (commRepoItem == false)
            //    {
            //        dgvCommissions.Width = 350;
            //    }
            //    else
            //    {
            //        dgvCommissions.Width = 415;
            //    }
            ...
            //    if (commRepoItem == false)
            //    {
            //        dgvCommissions.Width = 395;
            //    }
            //    else
            //    {
            //        dgvCommissions.Width = 545;
            //    }
            //}
            if (commItemStr == "Terms Type")
            {
        ...
        private void LoadSpiffs()
        {
            ...
            //commRepoItem = false; //testing only
            //commPerAccType = false; //testing only
            //SpiffPerBranch = false; //testing only
            spiffRepoGroup.Visible = commRepoItem && spiffTypeStr == "Single Spiffs";
            ...
            DataSet ds = PaymentManager.GetSalesCommissionRates(spiffTypeStr, selectDate, out _errorTxt);
                ...
                else
                {
                    ...
                    dgvSpiffs.Columns[CN.ItemText].Width = 85;
                    // Set visibility of Cash percentage column based on Country parameter 
                    ...
                    dgvSpiffs.Columns[CN.RepoPercentageCash].Visible = false;       // RI
                    dgvSpiffs.Columns[CN.RepoPercentageCash].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dgvSpiffs.Columns[CN.RepoPercentage].Width = 65;                                                        // RI
                    dgvSpiffs.Columns[CN.RepoPercentage].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dgvSpiffs.Columns[CN.RepoPercentage].HeaderText = GetResource("T_PctRepo");
                    dgvSpiffs.Columns["RepoValue"].Width = 65;
                    dgvSpiffs.Columns["RepoValue"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dgvSpiffs.Columns["RepoValue"].HeaderText = GetResource("T_ValueRepo");
                    // Set visibility of Repo percentage column based on Country parameter
                    dgvSpiffs.Columns[CN.RepoPercentage].Visible = commRepoItem;
                    dgvSpiffs.Columns["RepoValue"].Visible = commRepoItem;
                }
            ...
            try
            {
                    ...
                    spiffTypeStr == "Terms Type Spiffs" && ValidSpiff())
                {
                    ...
                    if (rowViewFound != null)
                    {
                        ...
                        rowViewFound[CN.RepoPercentage] = tbSpiffPcentRepo.Text;                // RI
                        rowViewFound[CN.RepoPercentageCash] = tbSpiffPcentRepo.Text;    
                        rowViewFound["RepoValue"] = Convert.ToDouble(tbSpiffValueRepo.Text);
                    ...
                    else
                    {
                        ...
                        rowView[CN.RepoPercentage] = tbSpiffPcentRepo.Text;                // RI
                        rowView[CN.RepoPercentageCash] = tbSpiffPcentRepo.Text;
                        rowView["RepoValue"] = Convert.ToDouble(tbSpiffValueRepo.Text);   
        ...
        private bool ValidSpiff()
        {
            ...
            if (tbSpiffValue.Text.Trim().Length == 0)
                tbSpiffValue.Text = "0";
            if (tbSpiffPcentRepo.Text.Trim().Length == 0)
                tbSpiffPcentRepo.Text = "0";
            if (tbSpiffValueRepo.Text.Trim().Length == 0)
                tbSpiffValueRepo.Text = "0";

            // Check Product item1 length       jec 16/0/08
            if (this.tbSpiffProduct.Text.Length > 12)
            {
            ...
            // Repo Spiff       // RI
            if ((!IsStrictNumeric(tbSpiffPcentRepo.Text) || tbSpiffPcentRepo.Text.Trim().Length == 0) && spiffTypeStr == "Single Spiffs")
            {
                errorProvider1.SetError(this.tbSpiffPcentRepo, GetResource("M_NUMERIC"));
                valid = false;
            }

            if (!IsStrictNumeric(tbSpiffValueRepo.Text) || tbSpiffValueRepo.Text.Trim().Length == 0)
            {
                errorProvider1.SetError(this.tbSpiffValueRepo, GetResource("M_NUMERIC"));
                valid = false;
            }
            ...
            if (valid)
            {
                ...
                // A percentage OR value must be entered - both can not be 0
                if (Convert.ToDouble(tbSpiffPcent.Text) == 0 && Convert.ToDouble(tbSpiffValue.Text) == 0 && commRepoItem == false)
                {
                    errorProvider1.SetError(this.tbSpiffValue, GetResource("M_PCENTANDVALUE"));
                    valid = false;
                }
                
                // Repo Spiff only percentage OR value may be entered not both
                if (spiffTypeStr == "Single Spiffs")
                {
                    if (Convert.ToDouble(tbSpiffPcentRepo.Text) != 0 && Convert.ToDouble(tbSpiffValueRepo.Text) != 0)
                    {
                        errorProvider1.SetError(this.tbSpiffValueRepo, GetResource("M_PCENTORVALUE"));
                        valid = false;
                    }
                    // A percentage OR value must be entered - both can not be 0
                    //if (Convert.ToDouble(tbSpiffPcentRepo.Text) == 0 && Convert.ToDouble(tbSpiffValueRepo.Text) == 0)
                    //{
                    //    errorProvider1.SetError(this.tbSpiffValueRepo, GetResource("M_PCENTANDVALUE"));
                    //    valid = false;
                    //}
                    if (Convert.ToDouble(tbSpiffPcentRepo.Text) == 0 && Convert.ToDouble(tbSpiffValueRepo.Text) == 0 && commRepoItem == true   // Repo% true
                                && Convert.ToDouble(tbSpiffPcent.Text) == 0 && Convert.ToDouble(tbSpiffValue.Text) == 0)                // and no comm entered for regular                
                    {
                        errorProvider1.SetError(this.tbSpiffValueRepo, GetResource("M_PCENTANDVALUE"));
                        valid = false;
                    }
                ...
                if (Convert.ToDouble(tbSpiffValueRepo.Text) > maxSpiffValue)
                {
                    errorProvider1.SetError(this.tbSpiffValueRepo, GetResource("M_COMMISSIONVALUEEXCEEDED", new object[] { maxSpiffValue }));
                    valid = false;
                }
            ...
            if (valid)
            {
                if (spiffTypeStr == "Linked Spiffs")
                {
                    ...
                    {
                        string check4 = PaymentManager.ValidateCommItem(spiffTypeStr, this.tbSpiffItem3.Text, dtpSpiffDateFrom.Value, dtpSpiffDateTo.Value, tbSpiffBranch.Text, out _errorTxt);
                    ...
                    {
                        string check5 = PaymentManager.ValidateCommItem(spiffTypeStr, this.tbSpiffItem3.Text, dtpSpiffDateFrom.Value, dtpSpiffDateTo.Value, tbSpiffBranch.Text, out _errorTxt);
        ...
        private void dgvSpiffsRowSelected_click(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                if (index >= 0 && spiffTypeStr == "Single Spiffs")
                {
                    ...
                    tbSpiffPcentRepo.Text = Convert.ToString(((DataView)dgvSpiffs.DataSource)[index][CN.RepoPercentage]);       // RI
                    tbSpiffValueRepo.Text = Convert.ToString(((DataView)dgvSpiffs.DataSource)[index][CN.RepoValue]);            // RI
                ...
                //  Clear Status message;
                if (btnEnterSpiff.Visible != false)     //Spiffs maintainable
                {
        ...
        private bool CheckForSave()
        {
            ...
            // #10951 Uplift changes
            if (btnSaveUplift.Enabled && !canceled)
            {
                DialogResult userResponse = ShowInfo("M_SAVECOMMCHANGES", new object[] { "Uplift" }, MessageBoxButtons.YesNoCancel);
                if (userResponse == DialogResult.Yes)
                {
                    btnSaveUplift_Click(null, null);
                }
                if (userResponse == DialogResult.No)
                {
                    // clear Data grid
                    dgvUpliftRates.DataSource = null;
                    btnSaveUplift.Enabled = false;
                }
                if (userResponse == DialogResult.Cancel)
                {
                    canceled = true;
        ...
        private void ClearErrors()
        {
            ...
            errorProvider1.SetError(this.tbCommPcentRepo, "");      // RI
            errorProvider1.SetError(this.tbCommPcentCashRepo, "");
            ...
            errorProvider1.SetError(this.tbSpiffValueRepo, "");     // RI
            errorProvider1.SetError(this.tbSpiffPcentRepo, "");           
    ...
    // This method overides the Common Form Method to catch FormClose when X clicked
        ...
        // Copy Credit% to Cash%
            ...
            if (cbCopyComm.Checked)
            {
                tbCommPcentRepo.Text = tbCommPcent.Text;
                tbCommPcentCashRepo.Text = tbCommPcentCash.Text;
            }
        ...
        private void tbCommPcentCash_Leave(object sender, EventArgs e)
        {
            if (cbCopyComm.Checked)
            {
                tbCommPcentCashRepo.Text = tbCommPcentCash.Text;
        ...
        private void tbSpiffPcent_Leave(object sender, EventArgs e)
        {
            if (cbCopySpiff.Checked)
            {
                tbSpiffPcentRepo.Text = tbSpiffPcent.Text;
            }
        }

        private void tbSpiffValue_Leave(object sender, EventArgs e)
        {
            if (cbCopySpiff.Checked)
            {
                tbSpiffValueRepo.Text = tbSpiffValue.Text;
            }
        }

        private void cbCopyComm_CheckStateChanged(object sender, EventArgs e)
        {
            // Copy Comm %/Value to Repo %/Value
            if (cbCopyComm.Checked)
            {
                tbCommPcentRepo.Text = tbCommPcent.Text;
                tbCommPcentCashRepo.Text = tbCommPcentCash.Text;
        ...
        private void cbCopySpiff_CheckStateChanged(object sender, EventArgs e)
        {
            // Copy Spiff %/Value to Repo %/Value
            if (cbCopySpiff.Checked)
            {
                tbSpiffPcentRepo.Text = tbSpiffPcent.Text;
                tbSpiffValueRepo.Text = tbSpiffValue.Text;
        ...
        private void tbCommPcentRepo_Leave(object sender, EventArgs e)
        {
            // Copy Credit% to Cash%
            if (cbCopyCredit.Checked)
            {
                tbCommPcentCashRepo.Text = tbCommPcentRepo.Text;
            }           
        }

        // #10951 Uplift Commission Rates
        private void LoadUpliftRates()
        {
            urloaded = false;
            uplift = PaymentManager.GetUpliftCommissionRates();         // #11203
            upliftRates = uplift.Tables[0];
            userRoles = uplift.Tables[1];

            DataRow newRow = userRoles.NewRow();        // #11203
            newRow["Name"] = "All Roles";
            newRow["Id"] = "0";
            userRoles.Rows.InsertAt(newRow,0);

            drpUserRole.DataSource = userRoles;
            drpUserRole.ValueMember = "Id";
            drpUserRole.DisplayMember = "Name";

            boundUpliftRates = new BindingSource();         // #11203
            boundUpliftRates.DataSource = upliftRates;

            dgvUpliftRates.DataSource = boundUpliftRates;       // #11203

            dgvUpliftRates.ColumnHeadersVisible = true;
            dgvUpliftRates.AutoGenerateColumns = true;

            dgvUpliftRates.Columns["Id"].HeaderText = "Employee Id";
            dgvUpliftRates.Columns["Id"].Width = 80;
            dgvUpliftRates.Columns["Id"].ReadOnly = true;
            dgvUpliftRates.Columns["Id"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgvUpliftRates.Columns["FullName"].HeaderText = "Employee Name";
            dgvUpliftRates.Columns["FullName"].Width = 180;
            dgvUpliftRates.Columns["FullName"].ReadOnly=true;
            dgvUpliftRates.Columns["UpliftCommissionRate"].HeaderText = "Uplift Commission % Rate";
            dgvUpliftRates.Columns["UpliftCommissionRate"].Width = 85;
            dgvUpliftRates.Columns["UpliftCommissionRate"].ReadOnly = false;
            dgvUpliftRates.Columns["UpliftCommissionRate"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgvUpliftRates.Columns["CurrentRate"].Visible = false;
            dgvUpliftRates.Columns["RoleId"].Width = 40;                    // #11203
            dgvUpliftRates.Columns["RoleId"].ReadOnly = true;
            dgvUpliftRates.Columns["RoleId"].Visible = false;
            dgvUpliftRates.Columns["Name"].HeaderText = "User Role";        // #11203
            dgvUpliftRates.Columns["Name"].Width = 150;
            dgvUpliftRates.Columns["Name"].ReadOnly = true;

            urloaded = true;
            
        }

        private void btnSaveUplift_Click(object sender, EventArgs e)
        {
            PaymentManager.SaveUpliftCommissionRates(upliftRates);
            btnSaveUplift.Enabled = false;
        }
        
        private void dgvUpliftRates_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsNumber(e.KeyChar))
                e.Handled = true;
        }

        private void dgvUpliftRates_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            errorProvider1.SetError(this.dgvUpliftRates, "");
            upliftError = false;

            if (e.ColumnIndex == 2)
            {
                var i = e.FormattedValue.ToString().Replace(".","");            // Input value  
                var c = Convert.ToString(dgvUpliftRates.Rows[e.RowIndex].Cells[3].Value).Replace(".", "");      // Current value
                var u = Convert.ToInt32(dgvUpliftRates.Rows[e.RowIndex].Cells[0].Value);      // user id

                int decPoints = e.FormattedValue.ToString().Split('.').Length -1;
                
                int number = 0;
                if (!int.TryParse(i.ToString(), out number) || decPoints > 1)
                {
                    //report about error input
                    errorProvider1.SetError(this.dgvUpliftRates, "Entered value invalid");
                    e.Cancel = true;
                    upliftError = true;
                }
                else
                {
                    if (Convert.ToDouble(e.FormattedValue) > 100 || Convert.ToDouble(e.FormattedValue)<0)
                    {
                        errorProvider1.SetError(this.dgvUpliftRates, "Value must be in range 0 to 100");
                        e.Cancel = true;
                        upliftError = true;
                    }
                    else
                    {
                        var y = Convert.ToDouble(e.FormattedValue);
                        var dpPos = e.FormattedValue.ToString().IndexOf(".");
                        var decPos = y.ToString().Length - (dpPos + 1);

                        if (decPos > 4)
                        {
                            errorProvider1.SetError(this.dgvUpliftRates, "Too Many decimal places");
                            e.Cancel = true;
                            upliftError = true;
                        }
                    }
                ...
                foreach (DataRow rowc in upliftRates.Rows)              // #11203 Update grid row for user in multiple Roles
                {
                    if (Convert.ToInt32(rowc["Id"]) == u)
                    {
                        //rowc["UpliftCommissionRate"] = Convert.ToString(i);
                        rowc["UpliftCommissionRate"] = Convert.ToString(e.FormattedValue);   //#16752
                ...
                if (i != c)
                {
                    btnSaveUplift.Enabled = !e.Cancel;          // enable/disable Save button
                    upliftUpdated = true;
                }
                else
                {
                    btnSaveUplift.Enabled = upliftUpdated;      // valid change previously
                }
            
            }
        }

        private void btnReloadUplift_Click(object sender, EventArgs e)
        {
            LoadUpliftRates();
            btnSaveUplift.Enabled = false;
            upliftUpdated = false;
        ...
        private void drpUserRole_SelectedIndexChanged(object sender, EventArgs e)       // #11203
        {
            if (urloaded == true)
            {
                selectedRole = Convert.ToInt32(drpUserRole.SelectedValue);

                boundUpliftRates.Filter = "RoleId = '" + selectedRole + "' or RoleId <> '" + selectedRole + "' and '" + selectedRole + "' = 0";
                dgvUpliftRates.DataSource = boundUpliftRates;
            }
