...
{
   /// <summary>
   /// Two int values created : 1 for DirectFileCheck and 2 for ServerManifestCheck
   /// </summary>
    ...
    [DebuggerHidden]
		...
		{  
				...
				if (ChangeDetectionMode == ChangeDetectionModes.ServerManifestCheck)
			{
				...
				//STL - changed so that a downgrade is possible also.
				retValue = vm.IsServerVersionDifferent(GetLatestInstalledVersion());
			} 
