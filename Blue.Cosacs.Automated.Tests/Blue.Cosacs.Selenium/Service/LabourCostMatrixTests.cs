...
{
    public class LabourCostMatrixTests
    {
        [Test]
        public void CreateLabourCostMatrixForProductGroup()
        {
            using (var session = Session.Get())
            {
                var webDriver = session.WebDriver;
                webDriver.GoTo("Service", "Labour Cost Matrix", "Service/Charges", session);
               // webDriver.WaitForElementPresent(By.CssSelector("[ng-model='labourItem.Label']"));
                ...
                webDriver.SelectSupplierInLabourMatrixForProductGroup("HP");
                webDriver.SelectRepairTypeInLabourMatrixForProductGroup("Assessment");
                webDriver.FillLabourCharges("10", "20", "30", "40");
        ...
        [Test]
        public void CreateLabourCostMatrixForSpecificProducts()
        {
            using (var session = Session.Get())
            {
                var webDriver = session.WebDriver;
               ...
               // webDriver.WaitForElementPresent(By.CssSelector("[ng-model='labourItem.Label']"));
                ...
                webDriver.SelectSupplierInLabourMatrixForSpecificProducts("HP");
                webDriver.SelectRepairTypeInLabourMatrixForSpecificProducts("Assessment");
                webDriver.FillLabourCharges("10", "20", "30", "40");
                Assert.IsFalse(webDriver.FindElement(By.CssSelector(".create [title='Save']")).GetAttribute("class").Contains("disabled"));
                webDriver.SaveMatrix();
            }
