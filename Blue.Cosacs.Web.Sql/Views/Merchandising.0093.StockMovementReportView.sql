...
--------- DIRECT TRANSFER SENDING LOCATION---------------
	...
	from
		...
		join Merchandising.Product p on tp.ProductId = p.Id and p.ProductType in ('RegularStock', 'RepossessedStock', 'SparePart')
		)
	UNION ALL
...
--------- ADJUSTMENT ---------------
	...
	from
		...
		join Merchandising.Product p on r.ProductId = p.Id and p.ProductType in ('RegularStock', 'RepossessedStock', 'SparePart')
		)
union all
--------- GOODS RECEIPT ---------------
	(select
	...
	from
		...
		join Merchandising.Product p on o.ProductId = p.Id and p.ProductType in ('RegularStock', 'RepossessedStock', 'SparePart')
	group by
		 x.id
		,x.LocationId
		,x.DateReceived
...
--------- GOODS RECEIPT (DIRECT)---------------
	...
	from
		...
		join Merchandising.Product p on y.ProductId = p.Id and p.ProductType in ('RegularStock', 'RepossessedStock', 'SparePart')
		)
union all
--------- VENDOR RETURN ---------------
	(select
...
(
	...
	INNER JOIN Merchandising.Product p 
		...
		AND p.Sku = x.Sku 
		AND p.ProductType IN ('RegularStock', 'RepossessedStock', 'SparePart')
