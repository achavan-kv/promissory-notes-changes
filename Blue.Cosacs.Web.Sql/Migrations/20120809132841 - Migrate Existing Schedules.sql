...
IF EXISTS(SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ScheduleToBookings]') AND type in (N'U'))
BEGIN
...
and l.quantity>0			-- existing order
AND NOT EXISTS(SELECT * FROM order_removed o WHERE o.acctno = s.acctno AND s.agrmtno=o.agrmtno and s.itemid=o.itemid and s.stocklocn=o.stocklocn) -- exclude removed schedules (data issue)
