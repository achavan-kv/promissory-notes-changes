...
SELECT acct.acctno + ' ' + CONVERT(VARCHAR, agreement.agrmtno)            AS invoiceNumber,
						   ...
						   ww.id										  AS warrantyId, 
						   WarrantyTable.itemno + 'M'                     AS WarrantyNumber,
						   Isnull(WarrantyTable.firstYearWarPeriod, 12)   AS WarrantyLengthFree,
						   WarrantyTable.warrantylength					  AS WarrantyLengthExtended,
...
LEFT OUTER JOIN 
	(SELECT warranty.contractno,
							   ...
							   wb.WarrantyLength - 12 AS warrantylength,
							   del.datedel
                          FROM   
								lineitem warranty
						  INNER JOIN 
							...
							INNER JOIN 
								warrantyband wb ON wb.ItemId = warranty.ItemID															
						 where warranty.quantity > 0
								AND warranty.ordval > 0)  AS WarrantyTable
...
INNER JOIN 
    ...
    AND item.itemtype = 'S'
INNER JOIN 
	warranty.warranty ww on ww.Number = WarrantyTable.itemno + 'M'
WHERE  NOT EXISTS (SELECT 1
                   FROM   cancellation
                   WHERE  cancellation.acctno = acct.acctno)
        AND WarrantyTable.itemid IS NOT NULL AND Dateadd(month, WarrantyTable.warrantylength, del.datedel) > Getdate()	
...
select 
		...
		t.Model,
		t.itemSupplier,
		t.WarrantyContractNo,
		t.warrantyId, 
		t.WarrantyNumber,
		t.WarrantyLengthFree,
		t.WarrantyTaxRate,
		t.WarrantCostPrice,
...
select 
		...
		w.Id, 
		left(t.WarrantyNumber, len(t.WarrantyNumber) - 1),
		t.WarrantyLengthExtended,
		t.WarrantyTaxRate,
		t.WarrantCostPrice,
		t.WarrantyRetailPrice,
		t.WarrantySalePrice,
...
from 
	#tempWarrantySale t
inner join 
	warranty.warranty w on w.Number = left(t.WarrantyNumber, len(t.WarrantyNumber) - 1)
