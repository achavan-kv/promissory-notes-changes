...
if NOT EXISTS(SELECT 1 FROM [$Migration] m WHERE m.Id = 20140519093617)
BEGIN 

	if not exists(select 1 from Warranty.Warranty where Number ='Manufacturer')
	begin
	...
	INSERT INTO  warranty.warrantysale
		(InvoiceNumber,  SaleBranch, SoldOn, SoldBy, CustomerAccount, CustomerId, CustomerTitle, CustomerFirstName, CustomerLastName, CustomerAddressLine1, 
		CustomerAddressLine2, CustomerAddressLine3, CustomerPostcode, ItemId, ItemNumber, ItemUPC, ItemPrice, ItemDescription, ItemBrand, ItemModel, ItemSupplier, 
		WarrantyContractNo, WarrantyId, WarrantyNumber, WarrantyLength, WarrantyTaxRate, WarrantyCostPrice, WarrantyRetailPrice, WarrantySalePrice, Status, 
		ItemSerialNumber, StockLocation, CustomerNotes, ItemCostPrice, ItemDeliveredOn, WarrantyGroupId, SoldById, ItemQuantity, EffectiveDate, WarrantyDeliveredOn, 
		WarrantyType, AgreementNumber)

	select 
		g.invoiceNumber,g.SaleBranch,g.SoldOn,g.SoldBy,g.CustomerAccount,g.customerId,g.customerTitle,g.CustomerFirstName,g.CustomerLastName,
	...
	from 
		...
		inner join stockinfo s 
			on d.itemno=s.itemno and s.itemtype='S'
		inner join #tmp g 
			on d.acctno=g.CustomerAccount 
			and d.agrmtno=g.AgreementNumber 
			and d.itemid=g.ItemId 
			and d.stocklocn=g.StockLocation,
		Warranty.Warranty w 
	where 
		d.agrmtno>0
		and not exists(select 1 from warranty.warrantysale ws where d.acctno=ws.CustomerAccount and d.itemno=ws.ItemNumber and d.agrmtno=ws.AgreementNumber)
		and not exists(select 1 from delivery c where d.acctno=c.acctno and d.itemid=c.itemid and d.stocklocn=c.stocklocn and c.delorcoll in('C','R') and d.agrmtno=c.agrmtno)
