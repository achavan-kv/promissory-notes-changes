 ...
 */
angular.module('ui.directives').directive('uiCodemirror', ['ui.config', '$timeout', 'codeeditor', function (uiConfig, $timeout, codeeditor) {
	'use strict';
	...
	return {
		...
		link:function (scope, elm, attrs, ngModel) {
			...
			deferCodeMirror = function () {
			    codeMirror = codeeditor.fromTextArea(elm[0], opts);
        ...
        if (controller) {
          var updateModel = function () {
            scope.$apply(function () {
              ...
              /*
                EXTREME HACK
                This is to try and work around the date getting converted to local time.
                Ends up losing a day, so set the hour value by the timezone offset, so 
                that we end up at 00:00 once the conversion to local time takes place.

                There may well be a better way to do this but I can't think of one.
              */
              var utcDate = new Date(0);
              var hours = date.getHours();
              if (date.getTimezoneOffset() > 0) {
                hours = (date.getHours() + (date.getTimezoneOffset()/60)) * 2;
              }
              utcDate.setUTCFullYear(date.getFullYear(), date.getMonth(), date.getDate());
              utcDate.setUTCHours(hours, 0, 0, 0);
              controller.$setViewValue(utcDate);
              element.blur();
            });
          };
          if (opts.onSelect) {
