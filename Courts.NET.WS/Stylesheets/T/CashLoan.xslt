  ...
  <xsl:template match="PAGE">
    <div style="position:relative">
      ...
      <table>
          ...
          <tr>
              <table width="100%">
                  <tr>
                      ...
                      <td align="right">
                          <b>
                              PRINCIPAL BORROWED: <xsl:value-of select="HEADER/LOANAMOUNT" />
                          </b>
                      </td>
                  </tr>
              </table>
        ...
        <tr>
          <td>To: UNICOMER ( Trinidad ) Limited (<xsl:value-of select="HEADER/PRINTBRANCH" /> Branch) (hereinafter referred to as the Lender)</td>
        </tr>
        <br/>
        <br/>
        <tr>
          <td>
            From: <xsl:value-of select="HEADER/NAME" /> 
          </td>
        </tr>
        <br/>
        <br/>
          <tr>
              <td>
            Of: <xsl:value-of select="HEADER/ADDR1" /> <xsl:value-of select="HEADER/ADDR2" /> (hereinafter referred to as the Borrower)
              </td>
          </tr>
        <br/>
        <br/>
        <tr>
          <td>
            For value received, the undersigned jointly and severally (if more than one), promise to pay UNICOMER ( Trinidad ) Limited (the Company), the sum of $<xsl:value-of select="../FOOTER/TOTAL" /> dollars including interests, payable by installments as follows:
          </td>
        </tr>
        <br/>
        <br/>
        ...
        <tr>
          <td>
            In the event the loan is prepaid within <xsl:value-of select="../FOOTER/EARLYSETTLEMENTPENALTYPERIOD" /> months of issuance, there shall be a prepayment penalty of up to two months interest of the prepaid portion.
          </td>
        </tr>
        <br/>
        <br/>
        ...
        <tr>
          <td>
            <table width="100%">
              ...
              <tr>
                <td>Witness (on behalf of UNICOMER ( Trinidad ) Limited)</td>
                <td>Date</td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          <td>
            I also authorize and instruct <xsl:value-of select="HEADER/EMPLOYER" /> &#160; <xsl:value-of select="HEADER/EMPYADD1" /> &#160; <xsl:value-of select="HEADER/EMPYADD2" /> &#160; <xsl:value-of select="HEADER/EMPYADD3" /> &#160; <xsl:value-of select="HEADER/EMPYPOCODE" /> and the Employer hereby agrees that on the receipt of the signed  Note by <xsl:value-of select="HEADER/NAME" /> (the Employee) to deduct the sum of <xsl:value-of select="../FOOTER/TOTAL" /> from the salary of <xsl:value-of select="HEADER/NAME" /> (the Employee) for remittance to UNICOMER ( Trinidad ) LIMITED in <xsl:value-of select="../FOOTER/INSTALNO"/> payments COMMENCING <xsl:value-of select="../FOOTER/INSTALTABLE/INSTALMENTDATE"/>, until the entire Note and related charges( if any) are paid off to UNICOMER ( Trinidad ) LIMITED
          </td>
        </tr>
        <br/>
        <br/>
