  ...
  <xsl:template match="PAGE">
    <div style="position:relative;">
      ...
      <table width="100%" style="font-size = 1.0em; margin: 0px; padding: 0px;" >
        ...
        <tr>
          <td>To: UNICOMER ( Jamaica ) Limited (hereinafter referred to as the Lender)</td>
        </tr>
        <br/>
        <br/>
        <tr>
            <xsl:variable name="jointName" select="HEADER/JOINTNAME" />
          <td>
            From: <xsl:value-of select="HEADER/NAME" /> of <xsl:value-of select="HEADER/ADDR1" /> <xsl:value-of select="HEADER/ADDR2" /> (hereinafter referred to as the Borrower)
              <xsl:if test="$jointName != ''">
                  and
                  <xsl:value-of select="HEADER/JOINTNAME" /> of <xsl:value-of select="HEADER/JOINTADDR1" /> <xsl:value-of select="HEADER/JOINTADDR2" />
              </xsl:if>
        ...
        <tr>
          <td>
            For value received the undersigned jointly and severally (if more than one), promise to pay UNICOMER ( Jamaica ) Limited trading as Courts, (the Company) the sum of <xsl:value-of select="../FOOTER/TOTAL" /> dollars being the principal sum of $<xsl:value-of select="../FOOTER/GOODSVAL" /> plus interest at the rate of <xsl:value-of select="../FOOTER/INTERESTRATE" /> payable by <xsl:value-of select="../FOOTER/INSTALNO + 1"/> installments as follows:
          </td>
        </tr>
        <br/>
        <tr>
        ...
        <tr>
          <td>
            In the event that a payment is overdue, late fees and interest will be assessed. In the event of  failure to pay as scheduled, I agree to pay any costs incurred in the collection of my debt, including but not limited to legal fees, bailiff fees and other collection expenses.
          </td>
        </tr>
        <br/>
        <br/>
        ...
        <tr>
          <td>
            <table width="100%" style="margin: 0px; padding: 0px;">
              ...
              <tr>
                <td>Witness (on behalf of UNICOMER ( Jamaica ) Limited)</td>
                <td>Date</td>
              </tr>
            </table>
          </td>
