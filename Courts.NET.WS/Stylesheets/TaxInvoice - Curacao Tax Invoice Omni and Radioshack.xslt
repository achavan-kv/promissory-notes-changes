...
Batteries, plastic / cosmetic parts and accessories. Low brightness on screen due to normal aging in the screen tube, lamp or burnt images
    ...
    <xsl:template match="FOOTER">
		<xsl:variable name="luxTaxes" select="sum(../LINEITEMS/LINEITEM[number(TAXRATE) &gt; 6]/TAXAMOUNTFULL)" />
		<xsl:variable name="obTaxes" select="sum(../LINEITEMS/LINEITEM[number(TAXRATE) = 6]/TAXAMOUNTFULL)" />
		<xsl:variable name="luxTaxesRel" select="sum(../LINEITEMS/LINEITEM/RELATED/LINEITEM[number(TAXRATE) &gt; 6]/TAXAMOUNTFULL)" />
		<xsl:variable name="obTaxesRel" select="sum(../LINEITEMS/LINEITEM/RELATED/LINEITEM[number(TAXRATE) = 6]/TAXAMOUNTFULL)" />	
        ...
        <TABLE class="normal" height="7" width="600" border="0">
            <TR>
                ...
                <TD width="20%"> <xsl:value-of select="format-number($luxTaxes + $luxTaxesRel, '#,##0.00')"/> </TD>
                <TD width="30%" align="right" class="bold">Sale subtotal: </TD>
                <TD width="20%" align="right">
                    <xsl:value-of select="EXTOTAL" />
                </TD>
            </TR>
            <TR>
                <TD width="30%" class="bold" rowspan="2">
                     OB Tax Total:
                </TD>
                <TD width="20%" rowspan="2"> <xsl:value-of select="format-number($obTaxes + $obTaxesRel, '#,##0.00')"/> </TD>                
