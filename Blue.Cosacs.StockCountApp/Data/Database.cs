...
namespace DapperLite
{
    ...
    public abstract class Database<TId>
    {
        ...
        private Dictionary<string, string> _tableNameMap;
        private readonly DapperLiteException _exceptionHandler;
        protected readonly bool _throwExceptions;
        ...
        private string GetTableName(MemberInfo type)
        {
            ...
            if (_tableNameMap.TryGetValue(typeName, out tableName))
            {
                return tableName;
            }

            // Then check if when the type name is pluralized, it matches a table name.
            if (_tableNameMap.TryGetValue(Pluralize(typeName), out tableName))
            {
                return tableName;
            }
