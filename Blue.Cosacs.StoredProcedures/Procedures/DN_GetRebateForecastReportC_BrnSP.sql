...
if exists (select * from dbo.sysobjects where id = object_id('[dbo].[DN_GetRebateForecastReportC_BrnSP]') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
drop procedure [dbo].[DN_GetRebateForecastReportC_BrnSP]
GO

CREATE PROCEDURE 	dbo.DN_GetRebateForecastReportC_BrnSP
-- ================================================
-- Project      : CoSACS .NET
-- File Name    : DN_GetRebateForecastReportC_BrnSP.prc
-- File Type    : MSSQL Server Stored Procedure Script
-- Title        : Rebate Forecast Report C by Branch
-- Author       : John Croft
-- Date         : 04/04/2008
--
-- This procedure will get the Rebate Forecast report C by branch.
-- 
-- Change Control
-- --------------
-- Date      By  Description
...
AS
	...
	SELECT	CONVERT(VARCHAR(11), period_end, 113) as periodend, branchno as BranchNo,
		...
		below_delivery_threshold, period_end
	INTO	#reportc
	...
	SELECT	periodend as PeriodEnd,
		...
		prior_month_forecast as Forecast, 
		...
		actual as Actual
		FROM	#reportc
		ORDER BY period_end DESC, BranchNo
