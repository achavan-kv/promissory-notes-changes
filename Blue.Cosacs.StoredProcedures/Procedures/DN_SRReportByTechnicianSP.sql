/****** Object:  StoredProcedure [dbo].[DN_SRReportByTechnicianSP]    Script Date: 10/23/2006 13:49:45 ******/
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('[dbo].[DN_SRReportByTechnicianSP]') AND OBJECTPROPERTY(id,'IsProcedure') = 1)
DROP PROCEDURE [dbo].[DN_SRReportByTechnicianSP]
GO
...
-- Description:	Service Requests by technician report
...
-- =============================================
CREATE PROCEDURE DN_SRReportByTechnicianSP
-- =============================================
-- Author:		Peter Chong
-- Create date: 22-Oct-2006
-- Title:	DN_SRReportByTechnicianSP
--
--	This procedure will retieve the details for By Technician.
-- 
-- Change Control
-----------------
...
BEGIN
	...
	(@MinSRNo IS NULL OR SR.ServiceRequestNo >= @MinSRNo) AND
	(@MaxSRNo IS NULL OR SR.ServiceRequestNo <= @MaxSRNo) AND
	ISNULL(DateClosed, '1900-01-01') = '1900-01-01'  --Date closed is blank 
END
