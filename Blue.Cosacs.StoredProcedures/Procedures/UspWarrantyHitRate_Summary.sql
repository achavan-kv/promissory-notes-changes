...
---------------------------------------------------------------------------------------------------------------------------------------------------
	--select '#AllWarrantyTransTest',* from #AllWarrantyTransTest
...
---------------------------------------------------------------------------------------------------------------------------------------------------
 ---  PRINT 'Get all Sold Warranties'
	...
	FROM Delivery d WITH(NOLOCK)
		...
		INNER JOIN #AllWarrantyTrans aws On d.acctno = aws.AccountNumber AND d.agrmtno = aws.AgreementNumber and d.ParentItemNo=aws.ItemNo
	...
	--select '#SoldWarranty',* from #SoldWarranty
	-------------------------------------------------------------------------------------------------------------------------------------------------
	...
	FROM Delivery d WITH(NOLOCK)
		...
		INNER JOIN [Merchandising].[ProductHierarchy] phc WITH(NOLOCK)	ON p.ID = phc.ProductId AND phc.HierarchyLevelId =3
		INNER JOIN [Merchandising].[HierarchyTag] htc WITH(NOLOCK) 	ON phc.HierarchyTagId = htc.id	AND htc.Name NOT IN ('AUDIO ACCESSORIES')
		INNER JOIN #AllWarrantyTrans aws On d.acctno = aws.AccountNumber AND d.agrmtno = aws.AgreementNumber and aws.ItemNo=d.ItemNo
		INNER JOIN acct a WITH(NOLOCK)      ON d.acctno = a.acctno
	...
	GROUP BY Saleperson,SalespersonName,Fascia
	--select '#SoldWarrantable',* from #SoldWarrantable
	--------------------------------------------------------------------------------------------------------
	...
	GROUP BY Saleperson,SalespersonName,Fascia 
	--select '#ReturnedWarranty',* from #ReturnedWarranty
	--------------------------------------------------------------------------------------------------------
	...
	WHERE ((d.delorcoll = 'C' AND contractno != '')) AND si.itemtype != 's' AND 1 = @IncludeReprocessedCancelled AND CONVERT(DATE, d.datetrans) BETWEEN @DateFrom AND @DateTo  
	--select '#ReturnWarrantyParent',* from #ReturnWarrantyParent 
	--------------------------------------------------------------------------------------------------------
	...
	--select '#RetrunedWarrantableWithinDate',* from  #RetrunedWarrantableWithinDate
	--------------------------------------------------------------------------------------------------------
	SELECT DISTINCT d.Acctno,d.Itemno, (d.quantity) As ReturnWarrantableQty,(d.transvalue) As ReturnWarrantableQtyValue,aws.Saleperson As Saleperson ,
		   aws.SalespersonName,aws.Fascia,CONVERT(DATE, d.datetrans) As datetrans
	...
	--------------------------------------------------------------------------------------------------------
	--select '#RetrunedWarrantableBeforeDate',* from #RetrunedWarrantableBeforeDate
	...
	GROUP BY Saleperson,SalespersonName,Fascia
	--select '#RetrunedWarrantable',* from #RetrunedWarrantable
	---------------------------------------------------------------------------------------------------------
	...
	FROM  #SalesPerson aw
		...
		LEFT JOIN #ReturnedWarranty tr ON aw.Saleperson = tr.Saleperson AND aw.SalespersonName = tr.SalespersonName AND aw.Fascia=tr.Fascia --where d.AcctNo='124002045981'
	GROUP BY aw.Saleperson,aw.SalespersonName,aw.Fascia
	--select '#WHRSummary',* from #WHRSummary
	-------------------------------------------------------------------------------------------------------------
	...
	SELECT Saleperson,SalespersonName,Fascia,
		WarrantableQty ,
		(CONVERT(DECIMAL(18,2),WarrantableQtyValue)) AS [WarrantableQtyValue] ,
		NoofWarrantySold ,
		(CONVERT(DECIMAL(18,2),SoldWarrantyValue)) AS [SoldWarrantyValue],
		NoofReturnWarranty ,
		(CONVERT(DECIMAL(18,2),ReturnWarrantableValue)) AS [ReturnWarrantableValue],
		NetWarrantySold,
		(CONVERT(DECIMAL(18,2),NetWarrantyValue)) AS [NetWarrantyValue],
