...
if exists (select * from dbo.sysobjects where id = object_id('[dbo].[DN_FintransGetCashierTotalsSP]') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
drop procedure [dbo].[DN_FintransGetCashierTotalsSP]
GO

CREATE PROCEDURE 	dbo.DN_FintransGetCashierTotalsSP
-- ================================================      
-- Project      : CoSACS .NET      
-- File Name    : DN_FintransGetCashierTotalsSP.sql      
...
--preventing system lock-up by reducing the number of days the fintrans query is running against
	...
	BEGIN
		...
		AND		FT.transtypecode in ('PAY', 'REF', 'COR', 'RET', 'DPY','CLD')			-- jec 12/10/11 Cash Loan Disbursement
	...
	BEGIN
		...
		BEGIN
			SELECT	FT.empeeno,
					...
					isnull(FTE.localchange,0) as localchange,
					A.securitised
			FROM		fintrans_new_income FT INNER JOIN code C	ON
					FT.paymethod = C.code AND
					C.category = 'FPM'  LEFT OUTER JOIN fintransexchange FTE ON
					FT.acctno = FTE.acctno AND
					FT.datetrans = FTE.datetrans AND
		...
		BEGIN
			SELECT	C.name,
					...
					isnull(FTE.localchange,0) as localchange,
					A.securitised
			FROM		fintrans_new_income FT INNER JOIN bank B	ON
