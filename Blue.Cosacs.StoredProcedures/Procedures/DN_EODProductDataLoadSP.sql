...
if exists (SELECT * FROM dbo.sysobjects WHERE id = object_id('[dbo].[DN_EODProductDataLoadSP]') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
drop procedure [dbo].[DN_EODProductDataLoadSP]
GO

CREATE PROCEDURE DN_EODProductDataLoadSP
-- ================================================
-- Project      : CoSACS .NET
-- File Name    : DN_EODProductDataLoadSP.prc
...
-- Date      By  Description
-- ----      --  -----------
-- 14/10/11  IP  #8405 - LW74090 - If Interface method is not FACT then we only need to import spare parts into CoSACS.
-- ================================================
        @return int OUTPUT
...
AS
	...
	(
		...
		suppliercode 	varchar(18) default '' not null,--3
	...
	SET @statement = 
	'BULK INSERT temp_prodload
	FROM '''+@drive+'\bmsfcprd.dat''
	WITH (
	DATAFILETYPE = ''char'',
	FORMATFILE = '''+@drive+'\bcp_bmsfcprd.fmt'') '

   EXEC sp_executesql @statement

	
	DELETE FROM temp_prodload 
	WHERE deleted = 'Y'
	AND EXISTS (SELECT 1 FROM temp_prodload	T
				WHERE T.deleted != 'Y'
				AND T.itemno = temp_prodload.itemno
				AND T.warehouseno = temp_prodload.warehouseno)	  -- ISSUE 69159 SC 27-7-07

	--IP - 14/10/11 - #8405 - LW74090
	IF(@InterfaceOption !='FACT')
	BEGIN
		DELETE FROM temp_prodload where category != 20
	END
