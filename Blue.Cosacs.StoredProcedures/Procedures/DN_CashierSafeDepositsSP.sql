...
if exists (select * from dbo.sysobjects where id = object_id('[dbo].[DN_CashierSafeDepositsSP]') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
drop procedure [dbo].[DN_CashierSafeDepositsSP]
GO

CREATE PROCEDURE 	dbo.DN_CashierSafeDepositsSP
			@empeeno int,
			@paymethod varchar(4), 
			@branchno smallint,
			@safedeposits money OUT,
			@return int OUTPUT
...
AS
	...
	/* find the sum of all deposits made to the safe since the cashier's session started */
	...
	IF(@paymethod = '')
	BEGIN	
		SELECT	@safedeposits = isnull(sum(depositvalue), 0)
		...
		SELECT	@safedeposits = @safedeposits + isnull(round(sum(CD.depositvalue * isnull(ER.rate, 1)),2),0)
		...
		AND		CD.IsFloat = 0			/* exclude floats */
