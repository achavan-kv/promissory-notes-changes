...
if exists (select * from dbo.sysobjects where id = object_id('[dbo].[DN_TelephoneActionLoadNewSP]') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
drop procedure [dbo].[DN_TelephoneActionLoadNewSP]
GO

-- =============================================
-- Author:		Jez Hemans
-- Create date: 31/05/2007
-- Description:	Retrieves all accounts in CMWorklistsAcct which the user is able to be assigned to and sorts them according to the date they entered the 
--              worklist (Datefrom) and the date an action was placed (dateadded).
-- =============================================
--exec DN_TelephoneActionLoadNewSP '99999',0

CREATE PROCEDURE 	dbo.DN_TelephoneActionLoadNewSP
            @user INT,
			@return int OUTPUT
...
--,
	...
	BEGIN
			...
			CU.custid, convert(datetime, null), 
--convert(datetime, null), '',
			'', '', '',wa.[Datefrom],MAX(b.[dateadded]),'', '', '',paidpcent
            FROM 	ACCT A, CUSTOMER CU, CUSTACCT CA, COURTSPERSON CP,
                [CMWorkList] w INNER JOIN [CMWorklistsAcct] wa ON [wa].[Worklist] = w.[WorkList]
                INNER JOIN [CMWorkListActions] wla ON [w].[WorkList] = wla.[WorkList]
                LEFT JOIN [bailaction] b ON  [b].[acctno] = wa.[acctno] 
            WHERE
             	--cp.[empeeno] = @user
              --      AND [w].[EmpeeType] = CP.empeetype
		            --and 	TODO TODO
		            A.Acctno = wa.acctno
		            and 	CA.Acctno = wa.acctno
		            and 	CA.hldorjnt = 'H'
		            and 	CU.custid = CA.custid
			    AND wa.Dateto IS NULL
			    AND wa.[acctno] NOT IN (SELECT [acctno] FROM [AccountLocking] WHERE CurrentAction = 'T')
            GROUP BY A.AcctNo, A.AcctType, A.DateAcctOpen, A.AgrmtTotal,
		    A.DateLastPaid,A.OutstBal, 
			A.Arrears,A.HighstStatus, A.CurrStatus, 
			CU.Title,CU.name, CU.Firstname,
			CU.custid,wa.[acctno],wa.[Datefrom],paidpcent

            ORDER BY [Datefrom],MAX([dateadded])
