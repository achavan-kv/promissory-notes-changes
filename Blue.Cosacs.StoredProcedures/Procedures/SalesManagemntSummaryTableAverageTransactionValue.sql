IF OBJECT_ID('SalesManagemntSummaryTableAverageTransactionValue') IS NOT NULL
	DROP PROCEDURE SalesManagemntSummaryTableAverageTransactionValue
GO

CREATE PROCEDURE SalesManagemntSummaryTableAverageTransactionValue
	@Today			Date
AS 
	SET NOCOUNT ON
	...
	(
		...
		SELECT 
			...
			SUM(CASE	
				WHEN CONVERT(Date, i.Datechange) = CONVERT(Date, @Today) 
					THEN (i.ValueAfter - i.ValueBefore)
				ELSE 0
			END) 
			/ 
			ISNULL(NULLIF(SUM(CASE	
				...
				ELSE 0
			END), 0), 1)													AS Amount,
			'Average Transaction Value'										AS Area
		FROM 
			LineitemAudit i
			INNER JOIN acct a
		...
		WHERE 
			i.acctno!= ''
			AND CONVERT(DATE, i.Datechange) = @Today
			AND (i.QuantityAfter - i.QuantityBefore) != 0
		GROUP BY 	
			i.Empeenochange
	) Data
