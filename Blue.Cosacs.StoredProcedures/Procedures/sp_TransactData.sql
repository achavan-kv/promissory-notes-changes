...
if exists (select * from dbo.sysobjects where id = object_id('[dbo].[sp_TransactData]') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
drop procedure [dbo].[sp_TransactData]
GO

CREATE PROCEDURE sp_TransactData
--------------------------------------------------------------------------------
--
...
--------------------------------------------------------------------------------
    ...
    @ProdCat            varchar(3) = null output, -- 68806 JH 16/10/2007
    ...
    @InstalPercentAllAccounts      FLOAT  = null output, --cr705
    -- MAIN Applicant
...
BEGIN
    ...
    SET @InstalPercentAllAccounts = 0.0
    ...
    where  CustId = @CustId
    and	   Acctno = @Acctno	
	 and not exists (select * from cancellation c where c.acctno =proposal.acctno)
    ...
    AND    s.ItemType  = 'S'		--CR 433 phase 2 - non stock items to not be included 27/09/04
   ...
   /*70795 Scoring now change in that HP accounts can get scored prior to items being selected
   - if this the case then use the product category selected from the sanction stage 1 screen
   if no stockitems selected  */
   if @SQLError = 0 and @ProdCat =' '
   begin
    ...
    Select @AllInstalAmounts = ISNULL(sum(InstalAmount),0)
    from   instalplan, custacct,acct
	 where  custacct.custid = @custid
    and instalplan.acctno = acct.acctno and custacct.acctno =acct.acctno
    and acct.currstatus !='S' and custacct.hldorjnt ='H'
    ...
    BEGIN
        Set @InstalPercent =  @InstalAmount / @NetIncome * 100
		  Set @InstalPercentAllAccounts=@AllInstalAmounts / @NetIncome * 100
    END
    ELSE
    BEGIN --income 0 so instalment percentage  infinity rounded down to 1000
        Set @InstalPercent = 1000
		  Set @InstalPercentAllAccounts= 1000
    END
