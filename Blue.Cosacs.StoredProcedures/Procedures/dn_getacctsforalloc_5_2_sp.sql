IF EXISTS (SELECT * FROM sysobjects
           WHERE xtype = 'P'
           AND name = 'dn_getacctsforalloc_5_2_sp')
BEGIN
	DROP PROCEDURE dn_getacctsforalloc_5_2_sp
END
GO 
  
CREATE procedure dn_getacctsforalloc_5_2_sp  
(  
 ...
 @worklist varchar(6),           -- CR852 JH 17/05/07  
     @deliveryarea varchar(8)        -- CR852 JH 17/05/07  
)  
AS   
        ...
        datelastpaid DateTime,  
         ...
         arrearsexcharges money,  
                arrearslevel float,  
                     ...
                     cusaddr3 varchar (50),   
                     postcode varchar (10),  
       DeliveryArea varchar(8),    -- CR852 JH 18/05/07  
                     instalamount money,  
                 dueday smallint,  
                     arrearsinst float,  
                     hldorjnt char(1),  
...
--here we are going to restrict the initial select   
   set @maxrow =15000  
   select @maxrow =isnull(maxrow,0) from courtsperson where empeeno =@user  
   if @maxrow =0  
     set @maxrow =10000  
...
set @columns = ' insert into #allocquery select  top ' + @top +    
   ' acct.acctno, acct.currstatus,acct.datelastpaid,' + @nl +  
    ...
    'convert (varchar (6), '''') as ethnicity, ' +   ' '''' as firstname, convert (varchar (50), '''') as cusaddr1, ' + @nl +  
     'convert (varchar (50), '''') as cusaddr2, convert (varchar (50), '''') as cusaddr3 , ' + ' convert (varchar (10), '''') as postcode,convert(varchar(8),'''') AS DeliveryArea, ' + @nl +  
...
begin  
    ...
    begin  
       ...
       set @tables =@tables + ' ,Admin.[User] au, Admin.UserRole ur'  
       set @where=@where + ' and follupalloc.empeeno =au.Id
                             and au.Id = ur.UserId
      ...
      if @empeeno !=0  
         ...
         set @where=@where + ' and au.Id =' + convert (varchar,@empeeno)  
...
/* restricting the selection by status code - either between or a single status code */  
    ...
    -- UAT 53 The default value for @statustype is '' not 'NL'  
    IF @statustype <> ''  
    begin  
...
execute dn_customerandaddressdetails5_2sp @table_name = '#allocquery',  
...
if @viewtop = 0  
  select a.acctno ,                   currstatus  as Status,  
    ...
    SR ,cashLoan               -- CR906 JH  
 from #allocquery a   
 WHERE DeliveryArea LIKE @deliveryarea  
else   
...
if @viewtop = 0  
  select a.acctno ,                   currstatus  as Status,  
       ...
       bdwcharges,  
             datestatchge,                --  jec 67911   
    SR  
from #allocquery a LEFT OUTER JOIN dbo.CMWorklistsAcct w ON a.acctno = w.acctno AND w.Dateto IS NULL  --#12857  
WHERE Worklist = @worklist AND DeliveryArea LIKE @deliveryarea  
else   
  select top 500 a.acctno ,                   currstatus as Status,  
                     outstbal  as Balance,  
                     Arrears ,  
...
WHERE reference = '1' AND category = 'SS1'  
