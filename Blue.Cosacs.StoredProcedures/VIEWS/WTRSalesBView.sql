IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[Merchandising].[WTRSalesBView]'))
	DROP VIEW [Merchandising].[WTRSalesBView]

/****** Object:  View [Merchandising].[WTRSalesBView]    Script Date: 11/22/2018 6:12:39 AM ******/
SET ANSI_NULLS ON
GO
...
AS
 ...
 ExportData (DateKey, TransactionDate, LocationId, LocationName, Fascia, SaleType, Division, DepartmentCode, Department, ClassCode, Class, Price, GrossProfit)
AS
(
SELECT CONVERT(INT, CONVERT(VARCHAR(8), DATEADD(mi, DATEDIFF(mi, GETUTCDATE(), GETDATE()), d.datetrans), 112)) AS DateKey
		  ...
		  ,CONVERT(DECIMAL(20,10),
            ...
            FROM country AS c with(Nolock), delivery d  with(Nolock)   
    ...
    LEFT OUTER JOIN Merchandising.ProductHierarchyView AS division with(Nolock)
	    ON division.ProductId = p.Id 
	    AND division.[Level] = 'Division' 
    LEFT OUTER JOIN  Merchandising.ProductHierarchyView AS Department with(Nolock)
	    ON Department.ProductId = p.Id 
	    AND Department.[Level] = 'Department'
    LEFT OUTER JOIN MERchandising.ProductHierarchyView as Class with(Nolock)
	    on Class.ProductId = p.Id
	    and Class.[Level] = 'Class' 
    INNER JOIN Merchandising.Dates AS dt 
    ...
    WHERE p.[Status] != (SELECT Id FROM Merchandising.ProductStatus with(Nolock) where Name = 'Non Active')
