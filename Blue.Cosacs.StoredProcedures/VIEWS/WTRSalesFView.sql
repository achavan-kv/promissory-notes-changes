IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[Merchandising].[WTRSalesFView]'))
	DROP VIEW [Merchandising].[WTRSalesFView]

/****** Object:  View [Merchandising].[WTRSalesFView]    Script Date: 11/27/2018 6:46:15 PM ******/
SET ANSI_NULLS ON
GO
...
AS
  ...
  ,
ExportData(DateKey, TransactionDate, LocationId, LocationName, Fascia, SaleType, Division, Price, GrossProfit)
    AS
    (
SELECT CONVERT(INT, CONVERT(VARCHAR(8), DATEADD(mi, DATEDIFF(mi, GETUTCDATE(), GETDATE()),  d.datetrans), 112)) AS DateKey
               ...
               , l.Fascia
			   , CASE 
					WHEN  d.acctno LIKE '___0%' THEN 'Credit'
					ELSE 'Cash'
				 END AS SaleType
