...
IF EXISTS(SELECT * FROM hub.[Queue] WHERE id = 5)
BEGIN
		UPDATE hub.[Queue] 
...
IF NOT EXISTS(select * from syscolumns
			where name = 'StockLocn'
			and object_name(id) = 'SR_Summary')
BEGIN

	alter table SR_Summary add StockLocn int not null default 0
...
BEGIN
	...
	alter table SR_Summary add Branch int not null default 0
