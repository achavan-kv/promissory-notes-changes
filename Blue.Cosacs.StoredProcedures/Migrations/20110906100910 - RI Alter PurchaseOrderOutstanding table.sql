...
--#8104 - UAT40 - Previously recieved duplicate key on insert when importing Purcahse Orders, therefore adding ID column and making as part of the key

IF NOT EXISTS(select * from syscolumns
			where name = 'ID'
			and object_name(id) = 'PurchaseOrderOutstanding')
BEGIN
	ALTER TABLE [dbo].PurchaseOrderOutstanding ADD [ID] [int] IDENTITY(1,1) NOT NULL
END
