...
select distinct '', '02', convert(varchar,stockinfo.category), isnull(codedescript, ''), '9', 'A' from stockinfo
inner join code on code.code = StockInfo.category and code.category in ('pcdis') 
union 
--Class
select distinct '', '03', convert(varchar,class), isnull(max(codedescript), ''), convert(varchar,stockinfo.category), 'A' 
from stockinfo
inner join code on code.code = StockInfo.category and code.category in ('pce', 'pcf', 'pcw', 'pco', 'pcdis')
